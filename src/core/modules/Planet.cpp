...
void Planet::init()
{
	...
	vMagAlgorithmMap.insert(Planet::Mueller_1893,	"Mueller1893");
	vMagAlgorithmMap.insert(Planet::Astr_Alm_1984,	"AstrAlm1984");
...
QString Planet::getInfoString(const StelCore* core, const InfoStringGroup& flags) const
{
	...
	if (flags&EclipticCoordJ2000)
	{
		...
		oss << q_("DEBUG: Ecliptical XYZ (VSOP87A): %1/%2/%3").arg(QString::number(eclPos[0], 'f', 7), QString::number(eclPos[1], 'f', 7), QString::number(eclPos[2], 'f', 7)) << "<br>";
	...
	if (flags&Extra)
	{
		...
		if (englishName != "Sun")
		{
			...
			if (englishName=="Moon" && core->getCurrentLocation().planetName=="Earth")
			{
				...
				else if (deltaLong<270)
					moonPhase = qc_("Waning Gibbous", "Moon phase");
				else if (deltaLong==270)
					moonPhase = qc_("Third Quarter", "Moon phase");
				else if (deltaLong<360)
					moonPhase = qc_("Waning Crescent", "Moon phase");
				// TRANSLATORS: <Moon Phase> at <deltaLongitude>°
				moonPhase=QString("%1 at %2°").arg(moonPhase, QString::number(deltaLong));
			}
